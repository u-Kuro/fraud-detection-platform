...
def test_load_reference_dataset(mocker: MockerFixture):
    mocker.patch(target="services.dags.drift_check.src.repositories.mlflow.registered_model_dataset.mlflow_module.artifacts.download_artifacts")

    table = pyarrow.table({
        TransactionInferences.transaction_timestamp.key: [1]
    })
    mocker.patch(
        target="services.dags.drift_check.src.repositories.mlflow.registered_model_dataset.parquet.read_table",
        return_value=table
    )
