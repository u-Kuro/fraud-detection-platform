...
from model_lifecycle_orchestrator.check_training_need.controllers.slack import build_training_approval_blocks_initializing, initialize_training_approval, cold_start_buttons, drift_retraining_buttons, build_training_approval_blocks
from model_lifecycle_orchestrator.check_training_need.modules.schemas.airflow.tasks import ModelDeploymentWorkflowForTraining
...
def test_initialize_training_approval(mocker: MockerFixture):
    ts = "value"
    mocker.patch(
        target="dags_shared.services.slack.slack_client.chat_postMessage",
        return_value={"ts": ts}
    )
