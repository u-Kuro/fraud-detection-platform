...
# def mock_k8s():
#     with patch("kubernetes.client.CoreV1Api", MagicMock()), \
...
# @pytest.fixture(autouse=True)
# def mock_postgres():
#     with patch("airflow.providers.postgres.hooks.postgres.PostgresHook.get_conn", MagicMock()):
#         yield
#
# @pytest.fixture(autouse=True)
# def mock_slack():
#     with patch("airflow.providers.slack.hooks.slack_webhook.SlackWebhookHook.send", MagicMock()):
#         yield

# def test_upload_to_s3():
#     with patch("airflow.providers.amazon.aws.hooks.s3.S3Hook.load_file",
#                return_value=MagicMock()):
#         from dags.my_dag import upload_file
#         upload_file.function("some/path")
#
# def test_call_api():
#     with patch("airflow.providers.http.hooks.http.HttpHook.run",
#                return_value=MagicMock(status_code=200, json=lambda: {"ok": True})):
#         from dags.my_dag import call_api
#         result = call_api.function()
